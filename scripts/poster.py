...
#------------------------------------------------------------------------#
def demoPSO() -> None:
    """Scenario 3: Multi-vehicle PSO Simulating"""
    printd("""
    ...
    This scenario demonstrates multiple AUVs using a PSO Algorithm to search
    the simulation for a source of pollution
    """)
    ...
    * Safety Distance (r_safe): 
    ...
    This scenario uses vehicles on equal standing to decide best course of action:
           
      * N Swarm Vehicles: Use PSO Guidance to find Source
    """)

    nVehicles = int(input("Number of vehicles (1-100) [3]: ").strip() or 3)
    nVehicles = max(1, min(nVehicles, 100)) #Will eventually change this line to allow for more vehicles
    numStr = f"{nVehicles}"
    print(f"> {nVehicles}")
    ...
    for veh in vehicles:
        veh.loadPSO()
        print(f"> Follower: \"{veh.callSign}\" APF target tracking")
    ...
    runTime = int(input(f"Simulation run time in (10-20,000) [Default 1,000]: ").strip() or 1000)
    runTime = max(10, min(runTime, 20000))
    print(f"> {runTime}")
    ...
    generatedName = '_'.join([name, numStr, attChoice, repChoice, formStr, commStr])
    name = input(f"Simulation name [{generatedName}]: ").strip()
    name = name.replace(' ', '_') if name else generatedName
    print(f"> {name}")
    ...
    print(f"  Number of vehicles:     {numStr}")
    print(f"  Followers:              APF guidance")
    print(f"  APF attraction:         PSOAtt")
    print(f"  APF repulsion:          PSORep")
    print(f"  Following distance:     {r_follow} m")
    print(f"  Avoidance distance:     {r_avoid} m")
    print(f"  Safety distance:        {r_safe} m")
    print(f"  Communication Network:  {commStr}")
    ...
    if confirm != 'n':
        ...
        sim = mn.simulator.Simulator(
            ...
            vehicles = vehicles,
            ocean = ocean,
            vehProxImmobilize=False,
        )

        gap = 700
        if (ocean.pollution.v * (180/np.pi)) < 90:
            xmin = int(ocean.size) / 2
            ymin = int(ocean.size)/2
            xmax = int(ocean.size)
            ymax = int(ocean.size)
        elif (ocean.pollution.v * (180/np.pi)) < 180:
            xmin = -int(ocean.size)/2
            ymin = int(ocean.size)/2
            xmax = -int(ocean.size)
            ymax = int(ocean.size)
        elif (ocean.pollution.v * (180/np.pi)) < 270:
            xmin = -int(ocean.size)/2
            ymin = -int(ocean.size)/2
            xmax = -int(ocean.size)
            ymax = -int(ocean.size)
        ...
        for auv in vehicles:
            x, y = 0, 0
            while dist([0,0],[x,y]) < gap: # keep generating points until one is far away enough
                x = xmin+(xmax - xmin)*np.random.rand(1)
                y = ymin+(ymax - ymin)*np.random.rand(1)
                x = int(x)
                y = int(y)
            sim.deployAtPoint(auv, x, y, 30)
...
def printSimDetailAnalysis(sim: mn.simulator.Simulator) -> None:
    ...
    for i, vehicle in enumerate(sim.vehicles):
        ...
        l.warning(f"Time to find Source: {vehicle.findSource}")
        #print(f"Time to find Source: {vehicle.findSource}")

        l.warning(f"Personal Best: ({vehicle.pbest[0]:.1f}, {vehicle.pbest[1]:.1f}, {vehicle.pbest[2]:.1f})m; concentration: {vehicle.pbest[3]}")
        l.warning(f"Group Best: ({vehicle.gbest[0]:.1f}, {vehicle.gbest[1]:.1f}, {vehicle.gbest[2]:.1f})m; concentration: {vehicle.gbest[3]}")
        #print(f"Personal Best: ({vehicle.pbest[0]:.1f}, {vehicle.pbest[1]:.1f}, {vehicle.pbest[2]:.1f})m; concentration: {vehicle.pbest[3]}")
        #print(f"Group Best: ({vehicle.gbest[0]:.1f}, {vehicle.gbest[1]:.1f}, {vehicle.gbest[2]:.1f})m; concentration: {vehicle.gbest[3]}")

...
def track_in(gotoservice1,interpolator, waypointpub):
	...
	if(not check_bounds(auv_location)): #hit boundary, reflect
		...
		wp1 = make_waypoint(new_waypoint[0], new_waypoint[1], new_waypoint[2])
		call_goto(wp1, gotoservice1, interpolator)
		waypointpub.publish(wp1)
...
def track_out(gotoservice1,interpolator, waypointpub):
	...
	if(particle_concentration >= THRESHOLD):
		...
		if(tout_init == 1): #determine destination waypoint
			...
			wp1 = make_waypoint(tout_wp[0], tout_wp[1], lost_pnts[-1][2])
...
def reacquire(gotoservice1,interpolator, waypointpub):
	...
	if(particle_concentration >= THRESHOLD):
		...
		if(bowtie_step == -1):	#go to center of bowtie
			bowtie_step = 0
			wp1 = make_waypoint(bowtie_center[0], bowtie_center[1], bowtie_center[2])
			...
			call_goto(wp2, gotoservice1, interpolator)
			waypointpub.publish(wp1)
			...
			if(has_reached(auv_location, bowtie_dright, R)): #end of bowtie maneuver reached without finding plume
				...
				if(len(lost_pnts) == 0): #no more ldp points to go through
					...
					find_plume(gotoservice1, interpolator)
				else:
					bowtie_step = -1

#Find behavior in algorithm
def find_plume(gotoservice1,interpolator, waypointpub):
	...
	if(particle_concentration >= THRESHOLD):
		...
		if(upnotcross == 1):
			...
			wp1 = make_waypoint(auv_location[0]+cross[0], auv_location[1]+cross[1], auv_location[2])
			call_goto(wp1, gotoservice1, interpolator)
			waypointpub.publish(wp1)
			...
			wp1 = make_waypoint(auv_location[0]+ufnorm[0], auv_location[1]+ufnorm[1], auv_location[2])
			call_goto(wp1, gotoservice1, interpolator)
			waypointpub.publish(wp1)
...
if __name__=='__main__':
	...
	nodeAddr = 1
	destAddr = 255 	# indicates the broadcast address, i.e. send to all the nodes in the network
	...
	# waypointpub = rospy.Publisher('aquanet_outbound_waypoint', Waypoint, queue_size = 1)
	# waypointpub = rospy.Publisher('Waypoint_pub', Waypoint, queue_size = 1)
	waypointpub = aquaNetManager
	...
	try:
		goto = rospy.ServiceProxy('rov1/go_to', GoTo)
	...
	while not rospy.is_shutdown():
		...
		if(alg_state == -1): #initial startup
			alg_state = 0
			prevstate = 0
			wp1 = make_waypoint(startx, starty, auv_location[2])
			call_goto(wp1, goto, interpolator)
			waypointpub.publish(wp1)

		elif(alg_state == 0):	#go to starting pos
			dest = np.array([startx, starty, auv_location[2]])
			if(has_reached(auv_location, dest, R)):
				find_plume(goto, interpolator, waypointpub)
		elif(alg_state == 1):
			prevstate = 1
			find_plume(goto, interpolator, waypointpub)
		elif(alg_state == 2):
			prevstate = 2
			track_in(goto, interpolator, waypointpub)
		elif(alg_state == 3):
			prevstate = 3
			track_out(goto, interpolator, waypointpub)
		elif(alg_state == 4):
			prevstate = 4
			reacquire(goto, interpolator, waypointpub)
			...
			if(not has_reached(auv_location, lost_pnts[-1], R)): #go to most upflow ldp, which is speculated source
				...
				wp1 = make_waypoint(lost_pnts[-1][0], lost_pnts[-1][1], lost_pnts[-1][2])
				call_goto(wp1, goto, interpolator)
				waypointpub.publish(wp1)
